 ...
 * Objetivo: Mover el dinero... exactamente 3 días después del pago.
 */
const startBalanceReleaser = () => {
  // Se ejecuta cada hora, en el minuto 0 (Ej: 1:00, 2:00, 3:00...)
  cron.schedule('0 * * * *', async () => {
    console.log('🏦 [CRON] Iniciando auditoría de liberación de fondos (3 días)...');

    try {
      // 1. Calculamos la fecha límite (hace exactamente 3 días / 72 horas)
      ...
      const pendingTransactions = await prisma.transaction.findMany({
        where: {
          ...
          createdAt: {
            lte: threeDaysAgo   // Su última actualización fue hace 3 días o más
          }
        }
      });
